...
public class OkraFry extends AppCompatActivity {

    private StorageReference reference;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_okra_fry);
        reference = FirebaseStorage.getInstance().getReference().child("OkraFry.PNG");

        //displays the current recipe from the database as an image file
        try {
            final File storedImage = File.createTempFile("OkraFry", "PNG");
            reference.getFile(storedImage).addOnSuccessListener(new OnSuccessListener<FileDownloadTask.TaskSnapshot>() {
                @Override
                public void onSuccess(FileDownloadTask.TaskSnapshot taskSnapshot) {
                    Toast.makeText(OkraFry.this, "Success",Toast.LENGTH_SHORT).show();
                ...
                public void onFailure(@NonNull Exception e) {
                    Toast.makeText(OkraFry.this, "Failed",Toast.LENGTH_SHORT).show();
                }
            });
        }
