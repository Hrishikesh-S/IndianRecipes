...
public class BroadBeansCurry extends AppCompatActivity {

    private StorageReference reference;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_broad_beans_curry);
        reference = FirebaseStorage.getInstance().getReference().child("BroadBeansCurry.PNG");

        //displays the current recipe from the database as an image file
        try {
            final File storedImage = File.createTempFile("BroadBeansCurry", "PNG");
            reference.getFile(storedImage).addOnSuccessListener(new OnSuccessListener<FileDownloadTask.TaskSnapshot>() {
                @Override
                public void onSuccess(FileDownloadTask.TaskSnapshot taskSnapshot) {
                    Toast.makeText(BroadBeansCurry.this, "Success",Toast.LENGTH_SHORT).show();
                ...
                public void onFailure(@NonNull Exception e) {
                    Toast.makeText(BroadBeansCurry.this, "Failed",Toast.LENGTH_SHORT).show();
                }
            });
        }
