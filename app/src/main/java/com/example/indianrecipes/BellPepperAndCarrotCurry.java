...
public class BellPepperAndCarrotCurry extends AppCompatActivity {

    private StorageReference reference;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_bell_pepper_and_carrot_curry);
        reference = FirebaseStorage.getInstance().getReference().child("BellPepperAndCarrotCurry.PNG");

        //displays the current recipe from the database as an image file
        try {
            final File storedImage = File.createTempFile("BellPepperAndCarrotCurry", "PNG");
            reference.getFile(storedImage).addOnSuccessListener(new OnSuccessListener<FileDownloadTask.TaskSnapshot>() {
                @Override
                public void onSuccess(FileDownloadTask.TaskSnapshot taskSnapshot) {
                    Toast.makeText(BellPepperAndCarrotCurry.this, "Success",Toast.LENGTH_SHORT).show();
                ...
                public void onFailure(@NonNull Exception e) {
                    Toast.makeText(BellPepperAndCarrotCurry.this, "Failed",Toast.LENGTH_SHORT).show();
                }
            });
        }
